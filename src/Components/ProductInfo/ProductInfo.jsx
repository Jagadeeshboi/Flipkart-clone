...
import { ExportApi } from "../../DataBase/Api";
...
const ProductInfo = () => {
  ...
  const RecomendationProducts = arrangedProducts.slice(0,8);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const products = await ExportApi(); 
        ...
        setArrangedProducts(transformedProducts); 
        setLoading(false); 
      } catch (error) {
  ...
  const product = arrangedProducts.find((item) => item.id === parseInt(productId, 10));
